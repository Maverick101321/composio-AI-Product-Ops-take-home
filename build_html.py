...
<body>
    <div class="container">
        ...
        <div class="patterns-section" style="border-left-color: #28a745;">
            <h3>The Pipeline</h3>
            <p>We built a multi-stage Python pipeline (<code>build_dataset.py</code> and <code>research_agent.py</code>) using the <strong>Composio SDK</strong> and the <strong>Google Antigravity Agent Framework</strong>.</p>
            <ol>
                <li><strong>Catalog Matching:</strong> First, the agent cross-referenced the 100 apps against the live Composio Toolkit catalog using <code>composio.toolkits.list()</code>. It matched 61 apps instantly, extracting their authentication schemes and base URLs.</li>
                <li><strong>Web Research:</strong> Next, we spawned asynchronous <code>app_researcher</code> subagents using the Antigravity SDK. The agents utilized web search tools to locate specific developer documentation and extract missing data (Self-serve status, blockers, MCP availability).</li>
                <li><strong>Human in the loop:</strong> A human was required to manually verify a random sample of the findings (e.g., confirming Binance's open-source MCP and Otter AI's enterprise-only API limit) and to resolve apps that blocked automated scraper traffic.</li>
            </ol>
